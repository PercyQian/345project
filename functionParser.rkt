  ...
  (lambda ()
    (let ((firstsymbol (get-next-symbol)))
      (cond
        ...
        ;; Accept other statements at the top level for backward compatibility with part1 and part2
        ((eq? (car firstsymbol) 'return)
         (let ((parse-statement (return-parse)))
           (if (eq? (car (get-next-symbol)) 'SEMICOLON)
               parse-statement
               (error 'parser "Missing semicolon"))))
        ((eq? (car firstsymbol) 'if)
         (unget-next-symbol)
         (if-parse))
        ((eq? (car firstsymbol) 'while)
         (unget-next-symbol)
         (while-parse))
        ((eq? (car firstsymbol) 'try)
         (unget-next-symbol)
         (try-parse))
        ((eq? (car firstsymbol) 'LEFTBRACE)
         (cons 'begin (compound-statement-parse)))
        ((eq? (car firstsymbol) 'break)
         (if (eq? (car (get-next-symbol)) 'SEMICOLON)
             (list 'break)
             (error 'parser "Missing semicolon")))
        ((eq? (car firstsymbol) 'continue)
         (if (eq? (car (get-next-symbol)) 'SEMICOLON)
             (list 'continue)
             (error 'parser "Missing semicolon")))
        ((eq? (car firstsymbol) 'throw)
         (let ((parse-statement (list 'throw (value-parse))))
           (if (eq? (car (get-next-symbol)) 'SEMICOLON)
               parse-statement
               (error 'parser "Missing semicolon"))))
        ((eq? (car firstsymbol) 'ID)
         (unget-next-symbol)
         (let ((parse-statement (simple-statement-parse)))
           parse-statement))
        (else
         (unget-next-symbol)
         (let ((parse-statement (simple-statement-parse)))
  ...
  (lambda ()
    (let ((name (get-next-symbol)))
      (if (and (eq? (car name) 'ID) (eq? (car (get-next-symbol)) 'LEFTPAREN))
          ...
          (error 'parser "Illegal start of function definition")))))
